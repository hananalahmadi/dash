...
# Define custom color palette function for RR
...
# Add yearly RR polygons to the map with custom colors
for (year in names(map_layers_rr)) {
  year_data <- map_layers_rr[[year]]
  ...
  leaflet_map_rr <- leaflet_map_rr %>%
    addPolygons(
      data = year_data,
      fillColor = ~sapply(RR, custom_palette), # Apply custom palette for RR
      color = "gray",
      weight = 1,
      opacity = 1,
      fillOpacity = 0.7,
...
leaflet_map_rr <- leaflet_map_rr %>%
  ...
  addLegend(
    position = "bottomright",
    colors = c("blue", "white", "red"), # Legend colors
    labels = c("RR < 1", "RR = 1", "RR > 1"), # Legend labels
    title = "RR",
    opacity = 1
  )
